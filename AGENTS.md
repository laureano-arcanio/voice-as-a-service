...
- **Server de validación (este):**
  ...
  - **Límites de GPU** (desde el 25-sep-2026): sin tope, las dos 3090 apagaron el server por un pico de consumo durante el test de capacidad.
    - Se aplican a mano con sudo: `nvidia-smi -pl 280`, techo del núcleo a 1800 MHz (`-lgc`) y memoria a 9501 MHz (`-lmc`).
...
- **Capacidad medida** ([CAP-001](docs/capacity/CAP-001-2x3090-pl280-classic/), con los límites de GPU):
  - ~20 llamadas con espera del cliente p95 ≤ 2,4 s; ~32 con p95 ≤ 3 s;
  - con ~32 se saturan las dos GPUs, y con 64 además la CPU del host (agente).
...
- **LLM 9B en 4 bits (EXP-009):** mejor que el 4B en los escenarios del motor y más rápido por turno. Capacidad medida en CAP-001 (antes EXP-012 y 013, archivados). El nombre del modelo en `VLLM_LLM_MODEL` tiene que coincidir entre `vllm-llm` y `app`/`agent`: al cambiarlo, recrear los tres.
...
- **Capacidad por motor en `.env`:** `VLLM_LLM_MAX_NUM_SEQS` (también fija el tamaño de CUDA graph), `VLLM_LLM_GPU_MEMORY_UTILIZATION`, `STT_MAX_BATCH`, `STT_MAX_BATCH_SECONDS`, `VLLM_TTS_GPU_MEMORY_UTILIZATION` y `VLLM_TTS_MAX_NUM_SEQS` (las dos etapas, por `--stage-overrides`). Los defaults del compose son la config vigente (EXP-013, CAP-001). Se aplican con `make up-inference`.
