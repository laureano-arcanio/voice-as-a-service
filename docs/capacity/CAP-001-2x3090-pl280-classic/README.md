...
- **Resultado:**
  - Piso con 1 llamada: espera p50 1,61 s / p95 1,99 s.
...
- **Límites de GPU:** un primer intento de `rampa` sin límites apagó el server con ~32 llamadas: las GPUs sumaban 600–660 W sostenidos, con picos de más. Con los límites, el máximo medido fue 559 W y no hubo cortes.
- **`base` repetido con los límites.** El primero, sin límites (`hw_id 351644b6`), dio un piso de p50 1,59 s / p95 1,89 s: con los clocks limitados el p95 sube ~0,1 s. Sus 3 saludos de más de 3 s eran arranques en frío y no se repitieron.

## Resultados

### Espera del cliente por escalón (`rampa`)

Espera: desde que el caller termina de hablar hasta el primer audio del agente. Agregada: p95 del escalón menos el p95 del `base` (1,99 s).

| Llamadas reales (objetivo) | Espera p50 / p95 | Agregada p95 | Sin respuesta | Saludo p50 / p95 | e2e server p50 / p95 | Buckets > 2 s |
|---|---|---|---|---|---|---|
| 7,2 (4) | 1,61 / 2,12 s | +0,12 s | 0 | 3,31 / 3,71 s (*) | 1,00 / 1,30 s | 13 % |
| 6,2 (8) | 1,58 / 2,12 s | +0,13 s | 0 | 0,97 / 2,79 s | 0,98 / 1,41 s | 9 % |
| 19,9 (16) | 1,86 / 2,38 s | +0,38 s | 0 | 1,32 / 3,12 s | 1,09 / 1,55 s | 35 % |
| **31,6 (32), codo** | **2,27 / 3,03 s** | +1,03 s | 0 | 1,54 / 3,86 s | 1,20 / 1,72 s | 71 % |
| 70,3 (64) | 5,83 / 8,25 s | +6,3 s | 7 | 4,71 / 6,84 s | 3,40 / 5,05 s | 99 % |
...
### Línea base (`base`, 1 llamada, con los límites de GPU)

| Métrica | Valor |
|---|---|
| Espera p50 / p95 / p99 | 1,61 / 1,99 / 2,17 s |
| e2e del server p50 | 0,90 s: eou 0,40 + STT 0,38 + LLM 0,13 + TTS 0,04 |
| Diferencia cliente − server | ~0,7 s |
| LLM | TTFT 126 ms, 11,9 ms entre tokens |
| TTS | 14,2 ms entre tokens |
| Saludo p50 / p95 | 0,91 / 1,41 s (máx. 1,59 s) |
| Llamadas buenas | 90 %; la que falla tiene 2 turnos de más de 2 s |
| WER | 0,12: "Sí." se transcribe como "C" |
| Duración media de llamada | 91,6 s |
| Agente | 0,12 cores y ~94 MB de PSS por job |
...
- **El saludo es lo primero que se degrada con la carga:** p95 1,41 s con 1 llamada, 3,1 s con 20 y 3,9 s con 32, contra 1,72 s del e2e. Arrancar el job (proceso y VAD) compite por la CPU.
...
Siguiente paso:
1. `fina` entre 20 y 40 llamadas, con escalones de 4 min.
2. Bajar el costo de arrancar un job (VAD en `prewarm`): es lo que degrada el saludo.
